...
if [[ -z "$PYTHON_BIN" ]]; then
  if command -v python3 >/dev/null 2>&1; then
    ...
    echo "Stage A e2e checks require python3." >&2
    exit 1
  fi
fi
...
PY
then
  echo "Stage A e2e checks require rdflib and pyshacl in the selected Python environment." >&2
  echo "Set PYTHON_BIN or install dependencies in .venv-ontology." >&2
  exit 1
fi

"$PYTHON_BIN" "$ROOT_DIR/compiler-mapping/scripts/run_stage_a_e2e_check.py"

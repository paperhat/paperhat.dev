...
2. **Arborist** - TypeScript/CDX parsing via SWC WASM (fast, 20-50x faster than
...
3. **Architect** - Semantic CDX compiling to HTML via RDF/SPARQL. Pipeline:
   `CDX → createElement → IR → Turtle → Oxigraph → SPARQL → JSON → render/renderToString → HTML`.
...
11. **Linguist** - Internationalization as triples. CDX authoring, ICU
...
12. **Sentinel** - Authentication/authorization as declarative data. OAuth2,
    ...
    auth for testing. CDX component API.
...
17. **Orchestrator** - Workflow automation as triples. n8n-style but CDX or
...
standards-compliant web applications as simple as writing semantic CDX. If it

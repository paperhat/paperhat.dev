...
```
CDX → IR → Triples → Oxigraph → SPARQL → JSON → render → HTML
```
...
```
CDX → IR → Turtle Triples → Oxigraph → SPARQL → JSON → render → HTML/DOM
```
...
| Library       | Purpose                                                                                                                                                |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Architect** | Context-aware semantic CDX compiling to HTML via RDF/SPARQL — 1000+ type-safe components with automatic accessibility and guaranteed WHATWG compliance |
| **Artificer** | Reactive behaviors as declarative CDX — calculations, validation, conditional display, schema-driven forms via SHACL                                   |
...
| **Orchestrator** | Workflow automation as triples — n8n-style pipelines with CDX or visual editing, distributed execution    |
...
Step order is automatic — child position in CDX becomes `schema:position` in
triples. No manual numbering. Reorder by moving elements.

That's it. Semantic CDX with plain English component names and props. No

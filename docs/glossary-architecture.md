...
Glossary terms are authored in TSX like everything else in Sitebender.
...
- Non-technical stakeholders need glossary access without touching TSX
...
- Everyone authors in TSX
- Git provides sufficient collaboration/versioning
- SPARQL meets all query needs

### Sync Direction

TSX is the source of truth. OpenMetadata syncs FROM Oxigraph, not the reverse.

```
TSX → Oxigraph → OpenMetadata (read-only view)
```

OpenMetadata provides a presentation layer for enterprise stakeholders while
actual authoring remains in TSX.
...
Sync to OpenMetadata for stakeholder presentations without exposing TSX.

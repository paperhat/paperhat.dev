...
- the interchange JSON encoding (see Behavior Program Encoding spec)
...
Behavior values are JSON-shaped.

The v0.1 type universe is:
...
- `object` (string keys to JSON values)

Evaluators MAY represent richer internal types, but MUST preserve observable behavior across runtimes.
...
- `Constant` has the type implied by the JSON value
...
- If either argument is `Invalid`, return `Invalid`.
- Otherwise compare JSON values for structural equality.
...
`IsInteger` MUST return `Valid(true)` only when the value is a JSON number with no fractional component.
...
The pattern language for v0.1 MUST be a restricted ECMAScript RegExp source (no flags).
...
In v0.1, a set value is represented as a JSON `array` whose elements are unique under structural equality.
...
- `IsMember(Validation<any>, Validation<array>) -> Validation<boolean>`
- `InSet(Validation<any>, Validation<array>) -> Validation<boolean>`
- `SetRel(Validation<array>, Validation<array>) -> Validation<boolean>`

Semantics:

1. If any argument is `Invalid`, return `Invalid`.
2. If a required set argument is not an `array`, return `Invalid([...])`.
...
- If the input is `Invalid`, return `Invalid`.
- `IsArray` returns true iff the value is a JSON array.
- `IsMap` returns true iff the value is a JSON object.
- `IsSet` returns true iff the value is a JSON array with no duplicates under structural equality.
...
In v0.1, temporal values MUST be represented as JSON strings.
...
- Presence/nullity: `IsNull`, `IsNotNull`, `IsPresent`, `IsAbsent`
...
- Behavior Program Encoding (interchange JSON form)

...
- **Pack root**: a directory containing one or more ontology packs (schema/dialect packs) loadable by Kernel as an explicit input.
- **Ontology pack**: a versioned directory of Codex artifacts authored in the Schema Dialect.
...
If `packs/` is present, it MUST be treated as a reserved directory for ontology packs and pack catalogs. It is not a Module root.
...
`packs/` is an optional conventional location for ontology packs.

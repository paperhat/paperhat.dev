...
### 4.4 Authoring-Time ID Minting (Permitted)

Tools MAY generate new `id` values at authoring time (including UUID-based values), provided that:

* the generated `id` is written into authored Codex and becomes an explicit input to Kernel
* Kernel does not mint new identities nondeterministically during compilation

This preserves determinism: given identical explicit inputs, outputs remain identical.

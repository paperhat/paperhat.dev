...
* dialect selection and pack loading rules
...
Given identical explicit inputs (including identical packs and their ordered roots), Kernel MUST produce identical semantic results.
...
* **Dialect pack** — a pack that provides one or more dialect ontologies and constraints. Dialect packs are authored in the Schema Dialect.
* **Vocabulary pack** — a pack that provides domain vocabulary within one or more dialects, without defining dialect meaning itself.

### 8.1 Pack Identity (Normative)

Each pack MUST declare, as authored Codex:

* a stable pack identifier
* an explicit pack version
* the dialect ontology(ies) and/or vocabulary it provides
* any dependencies on other packs (by identifier and version)

### 8.2 Pack Roots (Normative)

Kernel MUST treat packs as explicit inputs.
...
Kernel MAY compile loaded packs into a canonical internal form for efficient validation and compilation.

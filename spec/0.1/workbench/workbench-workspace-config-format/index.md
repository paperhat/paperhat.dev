...
- configure preview mode behavior (including a local preview surface)
...
- [Workbench Preview Mode](../workbench-preview-mode/)
...
- `.paperhat/runtime/` — tool-owned runtime artifacts (caches, state)

Build outputs go to `output/` at the workspace root (sibling to `modules/`), organized by target:

- `output/{target}/` — build outputs for each target (e.g., `output/html/`, `output/pdf/`)
...
## 8. Preview Mode Requirements (Normative)

In preview mode, Workbench MUST support:
...
- recompile config and update preview mode behavior deterministically, OR
- refuse with a diagnostic that a restart is required

---

## 9. Skipping Persistence in Preview Mode (Normative)

Workbench preview mode MUST support a mode that disables persistence steps intended for publish/build flows.

For example, in preview mode, Workbench MAY skip writing triples to a persistent graph store.
